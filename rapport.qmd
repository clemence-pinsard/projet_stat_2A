...
\noindent Pour trouver la position du pic, on annule la dérivée $P′(t)=ab \cdot e^{−bt}+cd \cdot e^{dt}=0$, ce qui donne directement : $t_{pic} = \frac {ln(\frac {ab} {cd})} {b+d}$. Pour obtenir la valeur au pic, on calcule P(t) pour $tt = t_{pic}$ et on obtient alors $P(t_{pic})=(a+c)−a \cdot (\frac {ab} {cd})^{\frac {-b} {b+d}}−c \cdot (\frac {ab} {cd})^{\frac d {b+d}}$.
...
\noindent Le critère retenu est le RMSE (Root Mean Squared Error), défini par : $$S(\theta) = \sqrt{\frac 1 N \sum_{i=1}^{N} (f_i - \hat{f}(age_i, \theta))^2}$$ On retient alors les paramètres qui minimisent ce critère : $$\hat{\theta} = \arg\min_{\theta} RMSE(\theta)$$
