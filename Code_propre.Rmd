...
stats_pic_equilibre <- stats_pic_discipline %>%
  group_by(discipline, Sex) %>%
  # On classe par la meilleure performance (attention au sens : décroissant pour sauts/lancers, croissant pour courses)
  # Pour simplifier, on utilise Best_Mark car tu l'as déjà calculée
...
ggplot(stats_pic_equilibre, aes(x = Age_Pic_Obs, fill = Sex)) +
  ...
  labs(
    title = "Distribution de l'âge au pic de performance (Top 39)",
    subtitle = "Comparaison à effectifs égaux | Lignes pointillées = Moyennes",
    x = "Âge au pic (années)",
    y = "Nombre d'athlètes",
    fill = "Sexe",
    color = "Sexe"
...
ggplot(stats_pic_equilibre, aes(x = Famille, y = Age_Pic_Obs, fill = Sex)) +
  ...
  scale_fill_manual(values = c("men" = "#2E86C1", "women" = "#28B463")) +
  labs(
    title = "Âge au pic selon les familles de disciplines (Top 39)",
...
ggplot(stats_pic_equilibre, aes(x = Age_Pic_Obs, fill = Sex)) +
  ...
  labs(
    title = "Allure de la distribution de l'âge au pic par discipline",
    subtitle = "Analyse de la normalité (Top 39 par sexe)",
    x = "Âge au pic (années)",
    y = "Densité"
  ) +
