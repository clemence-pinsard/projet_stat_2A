...
## Planche normalité
```{r}
# --- PLANCHE HISTOGRAMME : HOMMES (Bleu + Cadre) ---
p_hist_age_hommes <- ggplot(subset(stats_pic_equilibre, Sex == "men"), aes(x = Age_Pic_Obs)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, 
                 fill = "#2E86C1", color = "white", alpha = 0.8) +
  geom_density(color = "#1B4F72", size = 0.8) +
  facet_wrap(~ Famille, scales = "free_y", ncol = 3) +
  labs(title = "Distribution de l'âge au pic des athlètes masculins",
       x = "Âge au pic (années)", y = "Densité") +
  theme_minimal() +
  theme(
    strip.text = element_text(face = "bold"),
    plot.title = element_text(face = "bold", size = 14),
    # Ajout du cadre autour de chaque facette
    panel.border = element_rect(colour = "black", fill = NA, size = 0.7),
    panel.spacing = unit(1, "lines")
  )

ggsave("outputs/hist_age_hommes.png", p_hist_age_hommes, width = 12, height = 8)

# --- PLANCHE HISTOGRAMME : FEMMES (Vert + Cadre) ---
p_hist_age_femmes <- ggplot(subset(stats_pic_equilibre, Sex == "women"), aes(x = Age_Pic_Obs)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, 
                 fill = "#28B463", color = "white", alpha = 0.8) +
  geom_density(color = "#186A3B", size = 0.8) +
  facet_wrap(~ Famille, scales = "free_y", ncol = 3) +
  labs(title = "Distribution de l'âge au pic des athlètes féminines",
       x = "Âge au pic (années)", y = "Densité") +
  theme_minimal() +
  theme(
    strip.text = element_text(face = "bold"),
...
ggsave("outputs/hist_age_femmes.png", p_hist_age_femmes, width = 12, height = 8)
```

## Quantile - quantile
```{r}
# --- PLANCHE QQ-PLOT : HOMMES (Bleu + Cadre) ---
p_qq_age_hommes <- ggplot(subset(stats_pic_equilibre, Sex == "men"), aes(sample = Age_Pic_Obs)) +
  stat_qq(color = "#2E86C1", size = 1.2) +
  stat_qq_line(color = "#1B4F72") +
  facet_wrap(~ Famille, scales = "free", ncol = 3) +
  labs(title = "Quantile-quantile de l'âge au pic des athlètes masculins",
...
ggsave("outputs/qqplot_age_hommes.png", p_qq_age_hommes, width = 12, height = 8)

# --- PLANCHE QQ-PLOT : FEMMES (Vert + Cadre) ---
p_qq_age_femmes <- ggplot(subset(stats_pic_equilibre, Sex == "women"), aes(sample = Age_Pic_Obs)) +
  stat_qq(color = "#28B463", size = 1.2) +
  stat_qq_line(color = "#186A3B") +
  facet_wrap(~ Famille, scales = "free", ncol = 3) +
  labs(title = "Quantile-quantile de l'âge au pic des athlètes féminines",
       x = "Quantiles théoriques", y = "Quantiles observés") +
  theme_minimal() +
  theme(
    strip.text = element_text(face = "bold"),
...
ggsave("outputs/qqplot_age_femmes.png", p_qq_age_femmes, width = 12, height = 8)
```
...
test_par_discipline <- stats_pic_equilibre %>%
  group_by(discipline) %>%
  summarise(
    # On vérifie si on a bien les deux sexes dans la discipline
    P_Value = if(n_distinct(Sex) == 2) {
      wilcox.test(Age_Pic_Obs ~ Sex)$p.value
    } else {
      NA_real_
    },
    Mediane_Hommes = median(Age_Pic_Obs[Sex == "men"], na.rm = TRUE),
    Mediane_Femmes = median(Age_Pic_Obs[Sex == "women"], na.rm = TRUE),
